...
List<Products> smart_list;

 HashMap<String, List<Products>> ee ;
 String key;
 int flag;
...
protected void processRequest(HttpServletRequest request, HttpServletResponse response)
    throws ServletException, java.io.IOException 
    {
        sc=request.getSession().getServletContext();
         ...
         pw.println("<td>"+"<a href='/login2/AddProductByAdminDesignServlet'><b>Add Product </b></a>"+"</td>"); //LOAD ADD PRODUCTS BY ADMIN DESIGN SERVLET
		 pw.println("<td>");
		 
		 /*
		 USER ENTER THE ID OF PRODUCT TO BE UPDATED AND UPDATE PRODUCT DESIGN BY ADMIN SERVLET WILL OPEN
		 */
         pw.println("<form method='post' action='/login2/UpdateProductDesignByAdminServlet'>"+"<p>Enter the Product IDyou like to update</p>");
		 ...
		 */
         pw.println("<form method='post' action='/login2/DeleteProductByAdminServlet' style='float:right','font-size:900%>"+"<p>Enter the Product ID you like to delete</p>");
         pw.println("<input type='text' name='productDeleteId'>"+"<input type='submit' name='submit' style='background-color:#c98e8e'>"+"</form>");
		pw.println("<td>"+"<a href='/login2/DataAnalyticsDesignServlet'><b>Data Analytics</b></a>"+"</td>"); //LOAD ADD PRODUCTS BY ADMIN DESIGN SERVLET
		 pw.println("<td>");		 
		 pw.println("</td>");
		 pw.println("</tr>");
		 pw.println("</table>");
