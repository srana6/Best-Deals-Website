/*
USER CLICKS ON TABLET IN HOME PAGE AND TABLET DATA STORE SERVLET IS CALLED
*/
...
public class TabletDataStoreServlet extends HttpServlet implements Serializable{
	...
	public void printHtmlPage(HttpServletRequest request,HttpServletResponse response) throws ServletException, java.io.IOException{
		...
		productList=SaxParser.m.get("Tablet");
		...
		pw.println("<body>"+
					...
					"<section id='content'>"+
					"<article>"+
					"<h2>CHOOSE YOUR TABLET TODAY</h2>");
					/*
					TRAVERSING THORUGH PRODUCT LIST WHICH HAS DATA OF TABLETS
					*/
					...
					*/
					pw.println("<form action='/login2/AddingIntoCartServlet' method='get' style='margin-bottom:20px;'>");
					...
					pw.println("<form action='/login2/WriteReviewServlet' method='get' style='margin-bottom:20px;'>");
					pw.println("<input type='hidden' name='productFormId' value='"+prod.productId+"' />");
					...
					//pw.println("<hr width='85%' size='4' align='center'");
					pw.println("<form action='/login2/ReadReviewServlet' method='get' style='margin-bottom:20px;'>");
					pw.println("<input type='hidden' name='productFormId' value='"+prod.productId+"' />");
